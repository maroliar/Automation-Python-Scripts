...
# Script que aciona o ESP8266_Desodorizacao mediante um horário determinado, e envia uma notificação via OneSignal ou Telegram. 
...
notification_method = ""
notification_method_path = "/CONFIGURATION/SCRIPTS/AutoDeodorizeBathRoom/NotificationMethod/"

act_time_list = []
act_time_list_path = "/CONFIGURATION/SCRIPTS/AutoDeodorizeBathRoom/TimeToAct/"

logging.basicConfig(filename='/home/pi/Scripts/python/logs/autoDeodorizeBathRoom.log', level=logging.DEBUG, format='%(asctime)s %(levelname)s %(funcName)s => %(message)s')
...
def load_config_local():
    try:
        ...
        notification_method = data["CONFIGURATION"]["SCRIPTS"]["AutoDeodorizeBathRoom"]["NotificationMethod"]
        act_time_list = str(data["CONFIGURATION"]["SCRIPTS"]["AutoDeodorizeBathRoom"]["TimeToAct"]).split(",") 
    ...
    while True:      
        try:
            ...
            if(current_time in act_time_list):
                desodorizar_banheiro()
                enviar_notificacao("Banheiro desodorizado conforme schedule de horário.")
...
def desodorizar_banheiro():
    ...
    try:
        ...
        logging.exception("Ocorreu um erro no Metodo desodorizar_banheiro() {}".format(e))
...
try:
    print("Iniciando script autoDeodorizeBathRoom.py")
    logging.debug("Iniciando script autoDeodorizeBathRoom.py")           
               
    os.system("notify-send 'Home Automation' 'Iniciando script autoDeodorizeBathRoom.py'")
